                ...
                <div class="col-12">
                    <div class="card mb-4">
                        ...
                        <form class="row g-3 fv-plugins-bootstrap5 fv-plugins-framework" method="POST"
                            action="{{ route('salereturns.store') }}" id="saleForm" enctype="multipart/form-data">
                            @csrf
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-md-2 fv-plugins-icon-container">
                                        ...
                                        <input type="text" class="form-control" id="datetimepicker9" required
                                            name="return_invoice_date" value="{{ date('d-m-Y') }}">
...
<script>
        ...
        document.querySelectorAll('.addrowcontainer').forEach(function(button) {
            button.addEventListener('click', function() {
                ...
                document.getElementById("addrow").insertAdjacentHTML('beforeend',
                    ...
                    `<div class="col-md-1 border p-2"><input class="form-control sale_amount" type="text" name="sale_amount${cIncr}" id="sale_amount${cIncr}" ></div>` +
                    `</div>`
                );
    ...
    function updateTotals() {
        ...
        gstRates.forEach(rate => {
            ...
            //   );
            if (cgstElement && sgstElement &&  businessState === "Tamil Nadu") {
                ...
                igstElement.value = igst.toFixed(2);
            }
        ...
        console.log(
            `Total Amount: ${totalPrice.toFixed(2)}, GST Total: ${gstAmount.toFixed(2)}, Amount Without GST: ${withOutGst.toFixed(2)}`
        );
    }
    ...
    `;
        ...
        container.find('input.imei-field').on('input', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data.party, function(key, value) {
                    $("#party").append('<option value="' + value.id + '">' + value.name +
                        '</option>');
                });
