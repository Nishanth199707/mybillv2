    ...
    <div class="content-wrapper">
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                @if (!empty($purchase->tax_amount_28_cgst))
                @else
                    $('.twentyeight').css({
                        'display': 'none'
                    });
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="py-3 mb-4"><span class="text-muted fw-light"></span> Edit Purchase</h4>
                ...
                <div class="col-12">
                    <div class="card mb-4">
                        ...
                        <form class="row g-3 fv-plugins-bootstrap5 fv-plugins-framework" method="POST"
                            action="{{ route('purchase.show', $purchase->id) }}" id="saleForm"
                            ...
                            <div class="card-body p-32" id="tblData">
                                <div class="row mb-3">
                                    <div class="col-md-4 fv-plugins-icon-container" id="tblDataparty">
                                        ...
                                        <div class="border p-2 input-group col-md-4">
                                            <input type="text" class="form-control party" id="party" required
                                                name="party" value="{{ $purchase->party }}">
                                            <input type="hidden" class="party" id="partyid" required name="partyid"
                                                value="{{ $purchase->id }}">
                                ...
                                <div class="col-md-4 fv-plugins-icon-container">
                                    ...
                                    <input type="text" id="formValidationName" class="form-control"
                                        name="purchase_no" value="{{ $purchase->purchase_no }}">
                                ...
                                @foreach ($purchasedetail as $key => $val)
                                    <div class="row">
                                        <div class="border p-2 input-group col-md-3" style="width: 25%;">
                                            ...
                                            <input type="hidden" name="purchase_detail_id{{ $key + 1 }}"
                                                value="{{ $val->id }}" />
                                            <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                        ...
                                        <div class="col-md-2 border p-2">
                                            <input class="form-control amountbox" tabindex="1" type="text"
                                                dataid="{{ $key + 1 }}" name="amount{{ $key + 1 }}"
                                                id="taxableamount{{ $key + 1 }}">
                                        </div>

                                        <div class="col-md-1 border p-2">
                                            <input class="form-control gstperc" dataid="1" type="text"
                                                name="gst{{ $key + 1 }}" id="gst{{ $key + 1 }}"
                                                value="{{ $val->gst }}">
                                            <input class="gstvaldata" type="hidden" dataid="{{ $key + 1 }}"
                                                name="gstvaldata{{ $key + 1 }}"
                                        ...
                                        <div class="col-md-1 border p-2">
                                            <input class="form-control saleamount hidden" tabindex="1"
                                                type="hidden" name="saleamount{{ $key + 1 }}_hidden"
                                                value="{{ $val->sale_price }}">
                                            <input class="form-control saleamount" tabindex="1" type="text"
                                                name="saleamount{{ $key + 1 }}"
                                                id="saleamount{{ $key + 1 }}" value="{{ $val->sale_price }}">
                                        </div>
                            ...
                            <div class="m-1  justify-content-between align-items-center">
                                ...
                                <div class="row">
                                    ...
                                    <!-- Total Amount: ₹ -->
                                    <input readonly type="hidden" class="totalstyle" name="totalAmountDisplay"
                                        id="totalAmountDisplay" value="{{ $purchase->totalAmountDisplay }}" />
                                        ...
                                        <div class="col-md-4 fv-plugins-icon-container">
                                            ...
                                            <select name="cash_type" required id="cash_type" class="form-select">
                                                ...
                                                @foreach ($cash_type as $key => $val)
                                                    <option @if ($purchase->cash_type == $key) selected @endif
        ...
        document.querySelectorAll('.addrowcontainer').forEach(function(button) {
            button.addEventListener('click', function() {
                ...
                document.getElementById("addrow").insertAdjacentHTML('beforeend',
                    ...
                    `<div class="col-md-2 border p-2"><input class="form-control amountbox" tabindex="1" type="text" dataid="${cIncr}" name="amount${cIncr}" id="taxableamount${cIncr}"></div>` +
        ...
        $('#saleFormSubmit').click(function(e) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#purchase_url').val("{{ route('purchase', ':purchase_id') }}".replace(':purchase_id', purchase_id));
                    // table.draw();
                    // location.href = "{{ route('purchase', ':purchase_id') }}".replace(':purchase_id', purchase_id);
                    // var redirectUrl = "{{ route('purchase', ':purchase_id') }}".replace(':purchase_id', purchase_id);
                    // location.href = redirectUrl;

                    $.ajax({
                        url: "{{ route('purchase', ':purchase_id') }}".replace(':purchase_id', purchase_id),
                        type: 'GET',
                        dataType: 'json',
...
<script>
document.addEventListener('DOMContentLoaded', function() {
    const saleAmountInputs = document.querySelectorAll('.saleamount');

    saleAmountInputs.forEach(function(input) {
        // Function to calculate total with 18% added
        const salePrice = parseFloat(input.value) || 0;
        const total = salePrice + (salePrice * 0.18);
        input.value = total.toFixed(2); // Set the calculated total in the input field
    });
});
